...
login=$(curl http://${target}/graphql \
  ...
  -H 'content-type: application/json' \
  -H 'user-agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/109.0.0.0 Safari/537.36' \
  --data-raw '{"operationName":"UpdateCurrentUser","variables":{"input":{"fullName":"admin","email":"'${ADMIN_EMAIL}'","password":"'${ADMIN_PASSWORD}'"}},"query":"mutation UpdateCurrentUser($input: UpdateCurrentUserInput) {\n  updateCurrentUser(input: $input) {\n    id\n    fullName\n    email\n    __typename\n  }\n}\n"}'